setwd("C:/H/Tanzania Price data/Datasets/downloaded_pdfs/January2026")
...
write.csv(combined_df, "Jan_2026.csv")

#read extracted data CSV File
dta <- read.csv("Jan_2026.csv")
head(dta)
...
dta[dta$Market %in% c("Sumbawanga ", "Sumbawangwa"), c("Market")] <- "Sumbawanga"
...
dta[dta$Market %in% c("Kirombelo"), c("Market")] <- "Kilombero"
...
write.csv(dta, "Jan_2026_coodinates.csv", row.names = FALSE)

dta2 <- read.csv("Jan_2026_coodinates.csv")
dim(dta2)
head(dta2)
table(dta2$Market)
sapply(dta2, class)
